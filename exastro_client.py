...
# ---------------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Operation and input row -- both verified against this install, rows
    # created and deleted again, so every rule below is ITA's answer rather
    # than the shape of an example payload.
    # ---------------------------------------------------------------------
    ...
    def host_groups(self) -> list[dict]:
        ...
        out: list[dict] = []
        for r in rows:
            name = str(r.get("hostgroup_name") or "").strip()
            if name and name not in [g["name"] for g in out]:
                out.append({"name": name, "id": str(r.get("hostgroup_id") or "")})
        return out

    def hosts_in_group(self, group: str) -> list[str]:
        """The host names inside one group. An empty group says so by being empty.

        Membership comes from the link menu, whose rows carry both
        `hostgroup_name` and `hostname`. Falling back to the whole device list
        would let a run bind a host the operator never chose, which is the kind
        of convenience this file has already been burned by.
        """
    ...
    def host_value(host: str) -> str:
        """The spelling the input column wants for one host: `[H]<name>`."""
    ...
    def _select_label(scheduled: str, operation_name: str) -> str:
        ...
        Measured: ITA stores `2026-09-14 09:07:28` and advertises the item to
        the minute as `2026-09-14 09:07_DemoFlow20260914090728`; the same
        string *with* seconds is refused as an invalid value. The seconds come
        off the value read back, not off the string sent -- the stored form is
        what ITA compares against.
        """
        s = str(scheduled or "").strip()
        return f"{re.sub(r'(\d{2}:\d{2}):\d{2}$', r'\1', s)}_{operation_name}"

    @staticmethod
    def _is_duplicate(message: object, field: str) -> bool:
        low = str(message).lower()
    ...
    def create_operation(self, movement_name: str, remarks: str = "",
        ...
        Name rule from the operator's own install: movement name + `%Y%m%d%H%M%S`
        (`DemoFlow202609101551`). The read-back is not decoration -- the input row
        addresses the operation by its stored schedule, and that arrives
        reformatted (`/` written, `-` read).
        """
        when = when or time.localtime()
        base = f"{movement_name}{time.strftime('%Y%m%d%H%M%S', when)}"
        scheduled = time.strftime("%Y/%m/%d %H:%M:%S", when)
        name, attempt = base, 0
        while True:
            attempt += 1
            try:
                ...
                # The stamp only resolves to a second, and "Create another" can
                # be clicked twice inside one. A suffix is visible in the
                # report; silently reusing someone else's operation is not.
        ...
        raise ExastroError(
            f"operation '{name}' was accepted but is not listed yet; ITA is "
            f"still writing it — run this again")

    def create_movement_input(self, movement_name: str, params: dict,
                              operation: dict, host_name: str,
                              remarks: str = "") -> str:
        """Create the movement's input row: values, operation, host.

        The input menu *is* the movement's parameter sheet -- ITA creates it
        alongside the movement with sheet_type "Parameter Sheet(Host/Operation)",
        which is why the columns are the role variables again and why the host
        and operation columns exist at all.
        """
        body = {}
        for key, value in (params or {}).items():
            if value is None:
                continue
            body[str(key)] = (value if isinstance(value, (str, int, float, bool))
                              else json.dumps(value, ensure_ascii=False,
                                              separators=(",", ":")))
        body["operation_name_select"] = operation["select"]
        body["host_name"] = host_name
        if remarks:
            body["remarks"] = remarks
        try:
            self._maintenance(movement_name, body)
        except ExastroError as exc:
            found = self._retry_select_format(movement_name, operation, exc)
            if found is None:
                raise
            body["operation_name_select"] = found
            self._maintenance(movement_name, body)
            return (f"OK: input row for '{operation['name']}' on {host_name}"
                    " [the operation's label was taken from a row ITA had "
                    "already accepted]")
        return f"OK: input row for '{operation['name']}' on {host_name}"

    def _retry_select_format(self, movement_name: str, operation: dict,
                             exc: object) -> str | None:
        """Re-derive the operation label from a row ITA already accepted.

        The label's exact spelling is ITA's, not this tool's, and it is built
        from a stored timestamp whose format is an account detail. If a refusal
        names the column, copy the format from any input row of this movement
        pointing at the same operation. Returns None when there is nothing to
        copy, so the operator sees the original refusal rather than a
        second guess.
        """
        if "operation_name_select" not in str(exc):
            return None
        for row in self.filter_records(movement_name):
            if str(row.get("operation_name_disp") or "") == operation.get("name"):
                found = str(row.get("operation_name_select") or "")
                if found:
...
class MockExastroClient(ExastroClient):
    ...
    # {group: [host]} in the shape host_link_list holds it.
    _HOSTS = {"DEMO_GROUP": ["demo_host_a", "demo_host_b"],
              "DEMO_GROUP_ZOS": ["demo_host_zos"]}
    ...
    def host_groups(self) -> list[dict]:  # noqa: D102
        return [{"name": g, "id": f"hg-{i}"}
                for i, g in enumerate(sorted(self._HOSTS))]
    ...
    def create_operation(self, movement_name: str, remarks: str = "",
        ...
        when = when or time.localtime()
        name = f"{movement_name}{time.strftime('%Y%m%d%H%M%S', when)}"
        stored = time.strftime("%Y-%m-%d %H:%M:%S", when)
        return {"name": name, "operation_id": f"op-{name}", "scheduled": stored,
                "select": self._select_label(stored, name)}

    def create_movement_input(self, movement_name: str, params: dict,
                              operation: dict, host_name: str,
                              remarks: str = "") -> str:  # noqa: D102
        return f"OK: input row for '{operation['name']}' on {host_name}"
