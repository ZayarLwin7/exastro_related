...
def create_everything(movement_name: str, role_name: str, sheet_name: str,
    ...
    if create_op:
        # The operation has to exist and be readable before the input row can
        # name it, and both come after the sheet work: an input row pointing at a
        # movement whose parameters failed to bind would run with defaults.
        made = {}

        def _operation():
            made["op"] = cl.create_operation(movement_name)
            return f"{made['op']['name']} @ {made['op']['scheduled']}"

        step("operation", f"Create Operation for '{movement_name}'",
...
@app.get("/api/host_groups")
def api_host_groups():
    """Host groups and the hosts inside each, for the Operation popup.

    Both levels in one answer rather than a second request when the group
...
def create():
    ...
    # The Operation/Input toggle. Off is the default and the meaningful state:
