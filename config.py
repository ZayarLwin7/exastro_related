...
# --- Operation and Input-row menus (read back from this install) -------------
# The Conductor operation table. Rows here are identified by `operation_id`, not
# `uuid` -- an input row's key is `uuid` -- which is why the two writes below do
# not share a PATCH payload builder.
OPERATION_MENU = os.getenv("EXA_OPERATION_MENU", "operation_list")
# Host groups, and the rows that tie a group to the hosts inside it. The input
# column wants one of two spellings, both verified on this install:
#     [H]localhost     a host            -- accepted
#     [HG]DEMO_GP      a whole group     -- accepted
#     [H]no_such_host                    -- refused: "is an invalid value"
